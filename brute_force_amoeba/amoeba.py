...
# ---- inlined utilities to avoid pulling in BerryPy via brute_force_SGBZ.winding ----
...
def _probe_zero_plateau_near_mu1(
    ...
    mu1, a1 is zero and there are no a2-crossing zeros.  The step sizes combine
    the final outer bisection bracket with a fixed probe radius, so very narrow
    plateaus are not skipped just because the fixed radius is too large.
    """
    ...
    eps_min = max(10.0 * xtol, 1e-12)
    eps_max = max(float(probe_radius), 4.0 * bracket_width, 100.0 * xtol, eps_min)
        ...
        if _a1_is_degenerate(zeros_mid):
            ...
            if a1_left * a1_right < 0:
                return _make_ronkin_result(
                    ...
                    mu1_mid - continuum_perturb,
                    mu1_mid + continuum_perturb,
                    a1_left, a1_right,
                    "degenerate_sign_change",
                )
    ...
    try:
        ...
        if (not amoeba_res["is_continuum"]) and (len(amoeba_res["zeros"]) == 0):
            ...
            plateau_info = {
                "status": "found",
                "found": True,
                "reason": "empty_zero_list",
                "points": [],
            }
            classification_reason = "empty_zero_plateau"
        else:
            is_amoeba = True
            plateau_info = {
                "status": "skipped",
                "found": False,
                "reason": "continuum_or_disabled",
                "points": [],
            }
            classification_reason = "continuum_or_nonempty_zeros"

            if plateau_check and (not amoeba_res["is_continuum"]):
                ...
                if plateau_info["found"]:
                    ...
                    classification_reason = "nonempty_zeros_no_plateau"
                else:
                    classification_reason = "nonempty_zeros_plateau_check_inconclusive"
