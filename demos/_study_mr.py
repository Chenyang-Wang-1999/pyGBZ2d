...
for E in [-0.001, 0.0, 0.001]:
    ...
    for n, ndata in merge_nodes:
        ...
        th1 = float(np.angle(b1)) % (2*np.pi)
        th2 = float(np.angle(b2)) % (2*np.pi)
    ...
    if merge_nodes:
        ...
        for j, mr in enumerate(zm.multiple_roots):
            mr_th1 = mr.theta1 % (2*np.pi)
            mr_b2 = mr.roots[0]
            best = None
            for n, ndata in merge_nodes:
                th1 = float(np.angle(ndata['beta1'])) % (2*np.pi)
                d = min(abs(th1 - mr_th1), 2*np.pi - abs(th1 - mr_th1))
                if best is None or d < best_d:
                    best, best_d = (n, ndata), d
            if best is not None:
                n, ndata = best
                print(f'    MR[{j}] theta1={mr_th1:.6f} → node theta1='
                      f'{float(np.angle(ndata["beta1"]))%(2*np.pi):.6f} '
                      f'(dist={best_d:.2e})')
    ...
    if len(merge_nodes) >= 2:
        ...
        for ci, comp in enumerate(comps_after):
            th1s = [np.angle(G_test.nodes[n]['beta1'])%(2*np.pi) for n in comp]
            th2s = [np.angle(G_test.nodes[n]['beta2'])%(2*np.pi) for n in comp]
